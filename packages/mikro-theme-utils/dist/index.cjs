...
function getThemeVariable(prefix, key, value) {
  ...
  return Object.entries(value).map(([k, v]) => getThemeVariable(`${prefix}-${name}`, k, v)).join("");
}
function isThemeValue(value) {
  return !/components/.test(value);
}
function getThemeVariables(theme) {
  return Object.entries(theme).filter(isThemeValue).map(([type, values]) => getThemeVariable(`--mikro`, type, values)).join("");
}
function mergeThemes(theme1, theme2) {
  ...
  return (0, import_lodash.default)({}, theme1, theme2);
}
